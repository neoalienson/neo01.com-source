---
title: >-
  C、Go、Java、Javascript、PHP、PythonとSwiftのお遊びベンチマークテスト
tags:
  - Swift
  - Apple
  - nodejs
...
lang: ja
excerpt: Javaが勝利？最適化フラグなしの7言語にわたるバブルソートパフォーマンス対決が驚きの結果を明らかにします。
---

## 物語はSwiftとGoから始まる

私はAppleのSwift言語の美しさがGoogleのGoよりも好きです。はい、主観的です。[AppleがSwiftをオープンソース化](https://github.com/apple/swift)したので、どの言語が速度の面で優れているかについて客観的なアイデアを得るために、簡単なベンチマークテストを実行することにしました。
...
上記のサンプルからどの言語が優れているかを判断するのは困難です。しかし、結果は非常に簡単に推測できます：
...
Swiftは5.85秒で完了しましたが、Goは0.74秒しか必要としませんでした。

## 他の言語

他の言語のパフォーマンスを知りたくなりました。各言語で3回テストを実行し、最初の実行からの時間を破棄します。結果は2回目と最後の実行の平均です。このテストはお遊びなので、これで十分です。

_警告：すべてのコンパイラ/インタープリタに最適化フラグを使用していません。これらのフラグを適用すると、大きな違いが生じます。このトピックについては深く掘り下げません。_

Javaが勝者です！少なくとも最適化フラグの設定方法を知らないダミーにとっては。
...
{% echarts %}
{
  ...
  xAxis: {
    type: 'category',
    data: ['Swift', 'Go', 'C (gcc)', 'C (clang)', 'Java', 'JavaScript', 'PHP', 'Python'],
    axisLabel: {
      rotate: 30
    }
  },
  ...
  series: [
    {
      name: 'バブルソート時間',
      type: 'bar',
      data: [5850, 740, 218, 193, 150, 890, 2100, 8500],
      itemStyle: {
        color: function(params) {
          const colors = ['#c23531', '#2f4554', '#61a0a8', '#91c7ae', '#d48265', '#749f83', '#ca8622', '#bda29a'];
...
GitHubのソース [https://github.com/neoalienson/c_java_javascript_go_php_python_swift](https://github.com/neoalienson/c_java_javascript_go_php_python_swift)
