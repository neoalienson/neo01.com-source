...
tags:
  - Design
  - Methodology
excerpt: "设计思维不仅仅是设计师的专利——它是一个以人为本的问题解决框架。了解同理心、实验和迭代如何改变你构建产品的方式。"
...
**设计思维方法**：
1. **同理心**：采访自由职业者。在他们的工作日跟随他们。发现他们讨厌时间跟踪，因为它打断了创造性流程。
2. **定义**：自由职业者需要一种在不打断专注的情况下跟踪计费小时的方法。
3. **构思**：头脑风暴超越传统计时器的解决方案——基于应用使用的自动跟踪、语音激活记录、一天结束时的重建。
4. **原型**：构建不同方法的纸质模型。创建一个在后台运行的简单自动跟踪器。
5. **测试**：将原型提供给自由职业者。了解到自动跟踪感觉侵入性，但一天结束时的重建效果很好。
6. **迭代**：根据反馈改进重建界面。再次测试。改进。
