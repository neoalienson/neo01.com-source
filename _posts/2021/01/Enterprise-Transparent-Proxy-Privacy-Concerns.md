...
{% mermaid %}sequenceDiagram
    ...
    Note over C: ✓ Valid! Corporate CA is trusted
...
**Windows**:
```
Location: certmgr.msc → Trusted Root Certification Authorities
...
**macOS**:
```
Location: Keychain Access → System → Certificates
...
5. Browser shows: ✓ Valid certificate, green padlock
```
...
**In Firefox**:
```
1. Click padlock → Connection secure → More information
2. View Certificate → Issued By
...
!!!danger "🚨 Different Issuer = Man-in-the-Middle"
...
✓ HTTP headers (cookies, user agent, referrer)
✓ POST data (form submissions, uploads)
✓ Passwords and credentials
✓ Credit card numbers
✓ Private messages (email, chat, social media)
✓ Medical information
✓ Financial data
```
...
4. If it's corporate CA → you're being intercepted
...
4. If certificate doesn't match → connection fails
...
5. Mismatch detected → connection rejected
...
!!!tip "💡 Connection Failures Indicate Protection"
...
```
1. Visit https://google.com → Check issuer
2. Visit https://github.com → Check issuer
3. Visit https://neo01.com → Check issuer
...
```
✓ All web browsing (HTTP and HTTPS)
✓ All email (corporate and personal webmail)
✓ All file transfers
✓ All instant messaging
✓ All video calls (metadata, possibly content)
...
```
❌ Don't: Log into personal bank on corporate laptop
❌ Don't: Check personal email on corporate WiFi
❌ Don't: Shop online using corporate network
❌ Don't: Access medical records at work

✓ Do: Use personal phone with cellular data
✓ Do: Wait until you're home
✓ Do: Use VPN on personal device (if allowed)
```
...
Avoid:
❌ SMS (not encrypted)
❌ Regular email (not end-to-end encrypted)
❌ Slack/Teams (company can access)
```
...
```
✓ All traffic on company-owned networks
✓ All activity on company-owned devices
✓ All communications using company accounts
✓ All data stored on company systems
```
...
```
✓ Clearly communicate SSL interception is in use
✓ Explain what is monitored and why
✓ Document in acceptable use policy
✓ Provide training to employees
✓ Offer alternatives for personal activities
```
...
```
✓ Encrypt monitoring logs
✓ Restrict access to security team only
✓ Implement audit logging (who accessed what)
✓ Automatic redaction of sensitive data (passwords, credit cards)
✓ Clear retention policies (delete after 90 days)
✓ Incident response procedures (when to investigate)
```

### Provide Alternatives

```
✓ Guest WiFi without SSL interception for personal devices
✓ Cellular reimbursement for personal activities
✓ Clear break time policies (personal use allowed)
✓ VPN bypass for specific use cases
```
