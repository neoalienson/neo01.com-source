...
excerpt: "在 RTGS 普及之前，我們基本上是靠著希望和夜間批次作業的勇氣來運行支付系統。想像一下：你是一名中型銀端的 on-call DevOps / 中介軟體工程師。"
lang: zh-TW
...
!!!anote "🏦 什麼是淨額結算？"
    **淨額結算**（也稱為延遲淨額結算或 DNS）是一個支付系統，其中交易在**預定間隔**內作為**淨部位**結算，交易會**累積一段時間**。
...
graph TD
    A1[銀行 A] -->|支付 100 美元 | A2((RTGS))
    A2 -->|支付 100 美元 | A3[銀行 B]
    A1 -->|支付 50 美元 | A2
    A2 -->|支付 50 美元 | A4[銀行 C]
...
```mermaid
sequenceDiagram
    participant A as 銀行 A
    participant B as 銀行 B
    participant C as 銀行 C
    participant NS as 淨額系統
