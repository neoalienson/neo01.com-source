...
!!!anote "🏦 什么是净额结算？"
    **净额结算**（也称为延迟净额结算或 DNS）是一个支付系统，其中交易在**预定间隔**内作为**净部位**结算，交易会**累积一段时间**。
...
graph TD
    A1[银行 A] -->|支付 100 美元 | A2((RTGS))
    A2 -->|支付 100 美元 | A3[银行 B]
    A1 -->|支付 50 美元 | A2
    A2 -->|支付 50 美元 | A4[银行 C]
...
```mermaid
sequenceDiagram
    participant A as 银行 A
    participant B as 银行 B
    participant C as 银行 C
    participant NS as 净额系统
