---
title: "MCP伺服器：連接AI與應用程式的橋樑"
date: 2025-03-12
categories: AI
tags: [AI, MCP, Integration]
excerpt: "探索模型上下文協定（MCP）伺服器如何革新AI整合，實現語言模型與外部工具、資料庫和API之間的無縫連接。"
lang: zh-TW
...
graph TB
    ...
    A -->|"Stdio<br/>（無認證）"| B
    A -->|"Stdio<br/>（無認證）"| C
    A -->|"HTTP + OAuth<br/>（Bearer權杖）"| D
    A -->|"HTTP + OAuth<br/>（Bearer權杖）"| E
    B -->|"檔案I/O"| F
    C -->|"Git命令"| G
    D -->|"SQL"| H
    E -->|"HTTP"| I
