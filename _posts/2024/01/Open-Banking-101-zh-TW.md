...
tags:
  - Open Banking
  - Fintech
excerpt: "開放銀行聽起來很技術性，但其實核心概念很簡單：你的金錢資料屬於你，不屬於銀行。就是這個簡單的想法改變了一切。"
lang: zh-TW
...
```mermaid
graph TD
    A[公司切換薪資提供者] --> B[新提供者請求開放銀行存取]
    B --> C[員工授予一次性許可]
    C --> D[提供者透過 API 驗證帳戶詳細資訊]
    D --> E[即時批量驗證 - 所有員工]
    E --> F[無縫遷移 - 無需手動輸入]
    F --> G[第一個發薪日無中斷運行]
...
```mermaid
graph TD
    A[供應商開具發票] --> B[發票透過開放銀行在平台上可見]
    B --> C[多個投資者看到驗證的發票]
    C --> D[競爭性投標早期支付]
    D --> E[供應商在數小時內獲得支付，而非數月]
    E --> F[投資者在到期日收集全額]
